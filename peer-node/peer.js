...
const PEER_ID = (process.env.PEER_ID || '').trim() || `peer-${process.pid}`;
const LISTEN_HOST = (process.env.LISTEN_HOST || '127.0.0.1').trim();
const LISTEN_PORT = Number((process.env.LISTEN_PORT || '4001').trim());
const BOOTSTRAP_URL = (process.env.BOOTSTRAP_URL || 'http://127.0.0.1:3000').trim();
...
const NO_BOOTSTRAP = (process.env.NO_BOOTSTRAP || '').trim() === '1' || (process.env.NO_BOOTSTRAP || '').trim() === 'true';
const WEB_PORT = Number((process.env.WEB_PORT || '0').trim());
